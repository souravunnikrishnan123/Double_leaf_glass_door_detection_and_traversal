...
class create_full_view_bird_eye_view_state(BaseState):
    ...
    def do_action(self, ctx: FrameContext):
        ...
        pipeline_full_image = BirdsEyePassabilityPipeline(keyword = "full_image_view")
        
        point_cloud_vertical_ratio, color_image_roi, bird_eye_view = pipeline_full_image.run(ctx.depth_image_in_meters, ctx.fx, ctx.fy, ctx.cx, ctx.cy, ctx.color_image, full_image_roi, door_depth=2.0)