...
# ---------------------------------------------------------
def backproject_depth_to_points(depth_m,
                                fx, fy, cx, cy,
                                max_depth=5.0,
                                roi=None,
                                subsample=1):
    ...
    - depth_m: 2D numpy array (H,W) with depths in meters; invalid = 0 or np.nan
    ...
    - subsample: integer stride to reduce point count (1 = full)
    Returns:
    ...
    # sample the depth image on the grid
    depth_sample = depth_m[vv, uu]  # shape (h_roi, w_roi)
