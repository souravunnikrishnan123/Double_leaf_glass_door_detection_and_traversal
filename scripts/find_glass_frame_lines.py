...
def filter_vertical_lines_glass_contact(lines, depth_of_each_lines, fx, glass_width_cm, center_frame_width_cm):
    ...
    # Sort lines left to right by mean x using NumPy. Sort lines left to right based on x coordinate avg. as there can be a a lot of same coordinate for a line, it can cause bias. hence it is better to take mean to sort the line
    ...
    depth_cm_arr = np.array(depth_of_each_lines, dtype=np.float32) * 100.0
    valid_mask = depth_cm_arr > 0
    required_pixel_gap_arr = np.where(valid_mask, (glass_width_cm / depth_cm_arr) * fx, np.nan)
    frame_pixel_gap_arr = np.where(valid_mask, (center_frame_width_cm / depth_cm_arr) * fx, np.nan)
    ...
    if n:
        ...
        filtered = [lines[i] for i in selected_idx.tolist()]
    ...
    paired_lines = get_paired_lines(filtered, frame_pixel_gap_scalar, line_to_depth)
